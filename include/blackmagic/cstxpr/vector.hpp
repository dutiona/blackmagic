...
public:
  ...
  // enabled for cstxpr use for std::array in this version of gcc. As of
  ...
  {
    if (t_pos >= m_size) {
      // This is allowed in cstxpr context, but if the cstxpr evaluation
...
{
  return cstxpr::equal(x.begin(), x.end(), y.begin(), y.end());
}
