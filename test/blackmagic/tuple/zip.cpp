...
{
  ASSERT_TRUE(tuple::equal(tuple::zip(std::make_tuple(1, 2, 3)),
                           std::make_tuple(std::make_tuple(1), std::make_tuple(2), std::make_tuple(3))));
}

TEST(Tuple_Zip, Empty)
{
  ASSERT_TRUE(tuple::equal(tuple::zip(), std::make_tuple()));
}

TEST(Tuple_Zip, EmptySingle)
{
  ASSERT_TRUE(tuple::equal(tuple::zip(std::make_tuple()), std::make_tuple(std::make_tuple())));
}

TEST(Tuple_Zip, EmptyMulti)
{
  ASSERT_TRUE(tuple::equal(tuple::zip(std::make_tuple(), std::make_tuple()),
                           std::make_tuple(std::make_tuple(), std::make_tuple())));
}

TEST(Tuple_Zip, Double)
{
  ASSERT_TRUE(tuple::equal(tuple::zip(std::make_tuple(1, 2, 3), std::make_tuple(1.0, 2.0, 3.0)),
